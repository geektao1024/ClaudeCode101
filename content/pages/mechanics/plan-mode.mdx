...
> Last checked on July 8, 2026. Official Claude Code documentation describes Plan Mode as a permission mode for read-only analysis. It can read files, search, and run read-only exploration commands, but it should not edit your source files before you approve the plan. UI labels and shortcuts may change, so verify the current behavior in your installed version.
...
If you are searching for `Claude planner`, `Claude Code plan mode`, `auto mode`, or `Claude automode`, keep the distinction clear: Plan Mode is for read-only planning before edits, while auto mode changes how permission decisions are handled. They solve different problems and should not be treated as interchangeable.
...
| Run read-only shell commands              | Allowed for exploration.       | Use them to inspect state, not to mutate the project.    |
