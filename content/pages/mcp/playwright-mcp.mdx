...
## Can Claude Code Take Pictures?

For web UI work, yes: Claude Code can capture browser screenshots when it has a browser automation path such as Playwright MCP. That is the accurate answer behind searches like "does Claude Code take pictures." The screenshot is evidence from a real browser, not a guess from reading JSX or CSS.

Be precise about the boundary:

| Need                                          | Best route                                     | Boundary                                                         |
| --------------------------------------------- | ---------------------------------------------- | ---------------------------------------------------------------- |
| Screenshot a local web page                   | Playwright MCP                                 | The app must be running and reachable.                           |
| Inspect a modal, menu, or mobile layout       | Playwright MCP with viewport changes           | Still verify console errors and interactions.                    |
| Compare a rendered screenshot to expectations | Playwright MCP plus a written checklist        | Claude may miss subtle visual defects without explicit criteria. |
| Analyze an arbitrary image file               | Use a client surface that supports image input | This is not the same as browser screenshot automation.           |
| Verify 3D, CAD, or canvas output              | Render it, then capture a screenshot           | Source code alone is not visual proof.                           |
