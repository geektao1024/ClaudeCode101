...
<Callout type="info">
  最后核查时间：2026 年 5 月 24 日。npm 显示 `@anthropic-ai/claude-code`
  的 `latest` 和 `next` dist-tag 为 `2.1.150`，`stable` 为 `2.1.142`。官方
  changelog 最新条目是 `2.1.150`，发布日期为 2026 年 5 月 23 日。
  Claude Code 发布很快，升级生产开发环境前请重新运行本文命令确认。
</Callout>
...
截至本页核查时，官方 changelog 顶部为 `2.1.150`，说明这次主要是内部基础设施改进；前一版 `2.1.149` 包含 `/usage` 分类展示、diff 键盘滚动、Markdown task list 渲染和多项权限/PowerShell/工作树相关修复。
...
截至 2026 年 5 月 24 日核查，npm 的 `latest` 和 `next` 为 `2.1.150`，`stable` 为 `2.1.142`。但版本变化很快，实际升级前请重新运行：
