...
<Callout type="info">
  最后核查时间：2026 年 7 月 7 日。npm 显示 `@anthropic-ai/claude-code` 的
  `latest` 和 `next` dist-tag 为 `2.1.202`，`stable` 为 `2.1.195`。官方
  changelog 最新条目是 `2.1.202`，发布日期为 2026 年 7 月 6 日。Claude Code
...
如果你的真实问题是该跑 `claude update`、`npm install -g` 还是 `npm update -g`，先看专门的 [Claude update 和 npm install 怎么选](/zh/faqs/claude-update-vs-npm-install)。
...
截至本页核查时，官方 changelog 顶部为 `2.1.202`。升级前先读 changelog，因为 Claude Code 的发布可能包含 CLI 行为、权限、模型、MCP、IDE、workflow 和安装器变化，版本号本身不是唯一重点。更早的 Opus 4.8 关键发布是 `2.1.154`，随后 `2.1.156` 修复了同周出现的 thinking-block 错误。
...
截至 2026 年 7 月 7 日核查，npm 的 `latest` 和 `next` 为 `2.1.202`，`stable` 为 `2.1.195`。但版本变化很快，实际升级前请重新运行：
...
看你最初怎么安装。native installer 用 `claude update`；npm 安装用 `npm install -g @anthropic-ai/claude-code@latest`。完整判断路径看 [Claude update 和 npm install 怎么选](/zh/faqs/claude-update-vs-npm-install)。不要混用多个安装器，否则容易出现版本不一致。
