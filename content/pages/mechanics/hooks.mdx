...
description: 'Claude Code hooks guide for lifecycle events, PreToolUse guardrails, PostToolUse checks, matchers, settings, outputs, permissions, and debugging.'
---
...
If you are looking for Claude Code hooks documentation, start by deciding the event first: `PreToolUse` for preventing risky actions, `PostToolUse` for checks after edits, and `Notification` for human attention. Then narrow the matcher before writing any script.
