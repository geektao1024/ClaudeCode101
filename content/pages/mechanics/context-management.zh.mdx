...
## Context window size 和 auto-compact

很多人搜索 “Claude Code context window size” 或 `context left until auto-compact`，其实是在找一个固定数字。但实战里更可靠的判断方式是看当前 session、model、provider 和配置，而不是背一个旧 token 数。

需要判断当前窗口状态时，用 `/context`。如果 Claude Code 提示接近 auto-compact，把它当成会话健康信号，不要误判成账号额度限制。你仍然可以继续工作，但下一步应该决定：主动 compact、清空会话，还是先减少已加载文件。

| 信号                              | 含义                                      | 处理方式                                               |
| --------------------------------- | ----------------------------------------- | ------------------------------------------------------ |
| `context left until auto-compact` | 会话接近上下文边界。                      | 先运行 `/context`，同任务继续时带明确指令 `/compact`。 |
| “Claude Code context window size” | 这是 model、provider 和 client 配置问题。 | 查当前文档和当前 session，不要沿用旧数字。             |
| 工具输出或长日志反复出现          | 命令输出正在挤占窗口。                    | 重跑聚焦命令，或只读取日志相关范围。                   |
| 旧计划反复影响当前任务            | 旧对话状态仍在主导回答。                  | 新任务用 `/clear`；同任务用只保留当前事实的 compact。  |
