...
> Last checked on July 7, 2026. Official Claude Code docs describe ultracode as part of dynamic workflows, which are still a research preview and require Claude Code `2.1.154` or later. npm currently reports `@anthropic-ai/claude-code` `latest` and `next` as `2.1.202`, and `stable` as `2.1.195`.
...
| `Claude Code ultracode thinking blocks` | Is the Opus 4.8 API error related?      | Update to the current latest or at least `2.1.156` if you hit the historical thinking-block API error. |
