...
<Callout type="info">
  Last checked on May 24, 2026. npm reports `@anthropic-ai/claude-code`
  `latest` and `next` as `2.1.150`, and `stable` as `2.1.142`. The official
  changelog currently starts with `2.1.150`, dated May 23, 2026. Claude Code
  ships quickly, so verify again before upgrading production developer machines.
</Callout>
...
At the time this page was checked, `2.1.150` was the newest changelog entry and contained internal infrastructure improvements. The previous `2.1.149` release included user-facing changes such as a more detailed `/usage` view, keyboard scrolling in diff detail view, GitHub-Flavored Markdown task list rendering, and several permission, PowerShell, and git worktree fixes.
...
As checked on May 24, 2026, npm reports `latest` and `next` as `2.1.150`, and `stable` as `2.1.142`. Always verify before upgrading:
