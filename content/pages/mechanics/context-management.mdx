...
## Context Window Size And Auto-Compact

People often search for "Claude Code context window size" or "context left until auto-compact" expecting one fixed number. In practice, the useful answer is operational: check the current session, model, provider, and configuration instead of relying on a memorized token count.

Use `/context` when you need to know what is filling the active window. If Claude Code warns that the session is near auto-compact, treat it as a session-health signal, not an account limit. You can keep working, but you should decide whether to compact intentionally, clear the session, or reduce loaded files before the next large step.

| Signal                             | What it means                                         | What to do                                                                        |
| ---------------------------------- | ----------------------------------------------------- | --------------------------------------------------------------------------------- |
| `context left until auto-compact`  | The session is approaching its context boundary.      | Run `/context`, then `/compact` with focused instructions if the task continues.  |
| "Claude Code context window size"  | A model, provider, and client configuration question. | Verify current docs and inspect the active session rather than using old numbers. |
| Repeated tool outputs or long logs | Output is crowding the window.                        | Re-run focused commands or inspect saved log ranges.                              |
| A stale plan keeps resurfacing     | Old conversation state still dominates.               | Use `/clear` for a new task or compact around current facts only.                 |
